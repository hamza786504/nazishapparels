...
export default function AddressBookPage() {
    ...
    // Raw addresses straight from the database — never mock/static data.
    const [rawAddresses, setRawAddresses] = useState([]);
    const [formData, setFormData] = useState(initialFormData);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [loadingAddresses, setLoadingAddresses] = useState(true);

    // Map DB address shape -> card shape used by the UI. `_key` is preserved so
    // edit/delete operations can target the real document.
    const mapAddress = (a) => ({
        _key: a._key,
    ...
    // Derived display list (purely from DB state).
    ...
    const loadAddresses = async () => {
        ...
        try {
            ...
            if (data.success && Array.isArray(data.addresses)) {
                setRawAddresses(data.addresses);
            } else {
                setRawAddresses([]);
            }
        } catch {
            // On error, show an empty book rather than any placeholder data.
            setRawAddresses([]);
        } finally {
    ...
    const handleEdit = (address) => {
        // `address` is the mapped display card; resolve the raw DB record so we
        // populate every field (incl. apartment) correctly.
        const raw = rawAddresses.find((r) => r._key === address._key);
        if (!raw) return;
        setFormData({
        ...
        setEditingAddress(raw._key);
        setShowForm(true);
        document.body.style.overflow = 'hidden';
    };

    // Handle delete address
    const handleDelete = async (addressKey) => {
        if (!window.confirm('Are you sure you want to delete this address?')) return;
        // Optimistic remove, then sync with the server
        setRawAddresses(prev => prev.filter(addr => addr._key !== addressKey));
        try {
            await fetch(`/api/account/addresses?key=${encodeURIComponent(addressKey)}`, {
                method: 'DELETE',
            });
        } catch {
