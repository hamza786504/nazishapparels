...
/**
 * Client shell for the customer (account) area.
 *
 * The server-side AccountLayout (this directory's layout.jsx) fetches the
 * admin theme + fonts and injects them; this shell provides the interactive
 * chrome (Navbar, Sidebar rail + mobile drawer, Footer) and the AuthGuard.
 */
export default function AccountLayoutClient({ children }) {
    ...
    return (
        <AuthGuard>
            <div className="min-h-screen flex flex-col storefront-theme">
                <style jsx global>{`
                    body {
                        background-color: var(--color-background, #f6fafe);
                        color: var(--color-on-surface, #181c1f);
