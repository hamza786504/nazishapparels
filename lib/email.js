...
export function buildOrderCancellationEmail({ order }) {
  const html = wrapEmail(
    'Order Cancelled',
    `
    <h2 style="margin:0 0 12px;font-size:20px;">Your order has been cancelled</h2>
    <p style="font-size:14px;line-height:1.7;margin:0 0 16px;">
      Order <strong>${escapeHtml(order.orderId)}</strong> has been cancelled. No further
      processing, packing or shipping will happen for this order.
    </p>
    <table style="width:100%;border-collapse:collapse;font-size:14px;margin-bottom:16px;">
      <tr>
        <td style="padding:10px 14px;border:1px solid #ddd;background:#f5f5f5;font-weight:bold;white-space:nowrap;">Order ID</td>
    ...
    ${
      order.paymentStatus === 'Paid'
        ? `<p style="font-size:14px;line-height:1.7;margin:0 0 16px;">
            Your payment for this order will be refunded to you shortly.
          </p>`
        : ''
    }
    <p style="font-size:13px;color:#888;margin:0;">
      If you cancelled by mistake or need help, reply to this email and our concierge
      team will assist you.
    </p>
    `
  );

  const lines = [`Your order ${order.orderId} has been cancelled.`];
  lines.push('');
  lines.push('No further processing, packing or shipping will happen for this order.');
  if (order.paymentStatus === 'Paid') {
    lines.push('');
    lines.push('Your payment for this order will be refunded to you shortly.');
  }
  lines.push('');
  lines.push('If you cancelled by mistake or need help, reply to this email.');
  const text = lines.join('\n');
  return { html, text };
}